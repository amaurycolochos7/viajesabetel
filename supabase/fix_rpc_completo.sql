-- ================================================================
-- FIX COMPLETO: Funciones RPC + Permisos + Grants
-- ================================================================
...
-- Grant de ejecución para usuarios anónimos y autenticados
...
-- 2️⃣ FUNCIÓN: get_package_reservations
CREATE OR REPLACE FUNCTION public.get_package_reservations()
RETURNS TABLE (
  id UUID,
  reservation_id UUID,
  package_type TEXT,
...
-- Grant de ejecución
GRANT EXECUTE ON FUNCTION public.get_package_reservations() TO anon;
GRANT EXECUTE ON FUNCTION public.get_package_reservations() TO authenticated;

-- 3️⃣ FUNCIÓN: create_package_reservation
CREATE OR REPLACE FUNCTION public.create_package_reservation(
...
-- Grant de ejecución
GRANT EXECUTE ON FUNCTION public.create_package_reservation(UUID, TEXT, INTEGER, TEXT) TO anon;
GRANT EXECUTE ON FUNCTION public.create_package_reservation(UUID, TEXT, INTEGER, TEXT) TO authenticated;

-- 4️⃣ FUNCIÓN: register_package_payment
CREATE OR REPLACE FUNCTION public.register_package_payment(
...
BEGIN
  ...
  BEGIN
    INSERT INTO package_payment_transactions (
...
-- Grant de ejecución
GRANT EXECUTE ON FUNCTION public.register_package_payment(UUID, NUMERIC, TEXT, TEXT, TEXT) TO anon;
GRANT EXECUTE ON FUNCTION public.register_package_payment(UUID, NUMERIC, TEXT, TEXT, TEXT) TO authenticated;

-- ================================================================
-- 5️⃣ VERIFICACIÓN: Listar todas las funciones RPC creadas
-- ================================================================
SELECT 
  routine_name,
  routine_type,
...
-- ================================================================
-- NOTA: Si ves los paquetes y reservaciones arriba, ¡FUNCIONA!
-- Después ejecuta: NOTIFY pgrst, 'reload schema';
-- ================================================================
